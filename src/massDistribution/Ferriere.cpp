...
Vector3d Ferriere::CMZTransformation(const Vector3d &position) const {
	...
	long double y = -position.y;

	long double xC = -50*pc;		//offset
	long double yC = 50*pc;
	long double sinTc = sin(70.*deg);
	long double cosTc = cos(70.*deg);
...
Vector3d Ferriere::DiskTransformation(const Vector3d &position) const {
	...
	long double alphaD = 13.5*deg;  // rotation arround x-axis
	long double sinAd = sin(alphaD);
	long double cosAd = cos(alphaD);
	long double betaD = 20.*deg;  // rotation arround y'-axis
	long double sinBd = sin(betaD);
	long double cosBd = cos(betaD);
	long double TettaD = 48.5*deg;  // rotation arround x"-axis
	long double sinTd = sin(TettaD);
	long double cosTd = cos(TettaD);
...
long double Ferriere::getHIDensity(const Vector3d &position) const {
	long double n = 0;
	long double R = sqrt(position.x*position.x+position.y*position.y);

	if(R<3*kpc)
	{
		// density at center
		Vector3d pos = CMZTransformation(position);  // coordinate trafo
		long double x = pos.x/pc;  // all units in pc
		long double y = pos.y/pc;
		long double z = pos.z/pc;

		long double A = sqrt(x*x+2.5*2.5*y*y);
		long double nCMZ = 8.8/ccm*exp(-pow_integer<4>((A-125.)/137))*exp(-pow_integer<2>(z/54.));
		...
		long double nDisk = 0.34/ccm*exp(-pow_integer<4>((A-1200.)/438.))*exp(-pow_integer<2>(z/120));

		n = nCMZ + nDisk;
	}
	else{  // outer region
		long double z = position.z/pc;
		long double a;
		if(R<=Rsun){
			a = 1;
		} else {
		...
		long double nCold = 0.859*exp(-pow_integer<2>(z/(127*a))); // cold HI
		nCold += 0.047*exp(-pow_integer<2>(z/(318*a)));
		nCold += 0.094*exp(-fabs(z)/(403*a));
		nCold *= 0.340/ccm/(a*a);

		long double nWarm = (1.745 - 1.289/a)*exp(-pow_integer<2>(z/(127*a)));  // warm HI
...
long double Ferriere::getHIIDensity(const Vector3d &position) const {
	long double n = 0;
	long double R = sqrt(position.x*position.x+position.y*position.y);

	if(R< 3*kpc){   // inner
		long double x = position.x/pc;
		long double y = position.y/pc;
		long double z = position.z/pc;

		// warm interstellar matter
		long double H = (x*x + pow_integer<2>(y+10.))/(145*145);
		long double nWIM = exp(-H)* exp(-pow_integer<2>(z+20.)/(26*26));
		...
		long double alphaVH = 21.*deg;  // angle for very hot IM in radiant
		long double cosA = cos(alphaVH);
		long double sinA = sin(alphaVH);
		long double etta = y*cosA+z*sinA;  // coordinate transformation for VHIM along major axis
		long double chi = -y*sinA+z*cosA;

		long double nVHIM = 0.29/ccm*exp(-((x*x+etta*etta)/(162.*162.)+chi*chi/(90*90)));

		n = nWIM + nVHIM;
	} else {  // outer region
		long double z = position.z;

		long double nWarm = 0.0237/ccm*exp(-(R*R-Rsun*Rsun)/pow_integer<2>(37*kpc))*exp(-fabs(z)/(1*kpc));
		nWarm += 0.0013/ccm*exp(-(pow_integer<2>(R-4*kpc)-pow_integer<2>(Rsun-4*kpc))/pow_integer<2>(2*kpc))*exp(-fabs(z)/(150*pc));

		long double nHot = 0.12*exp(-(R-Rsun)/(4.9*kpc));
...
long double Ferriere::getH2Density(const Vector3d &position) const{
	long double n=0;
	long double R=sqrt(position.x*position.x+position.y*position.y);
	...
	if(R<3*kpc) {
		...
		long double A = sqrt(x*x+pow(2.5*y,2));  // ellipticity
		long double nCMZ = exp(-pow((A-125.)/137.,4))*exp(-pow(z/18.,2));
		...
		long double nDISK = exp(-pow_integer<4>((A-1200)/438))*exp(-pow_integer<2>(z/42));
...
long double Ferriere::getDensity(const Vector3d &position) const{
	long double n=0;
	if(isforHI){
...
long double Ferriere::getNucleonDensity(const Vector3d &position) const{
	long double n=0;
	if(isforHI){
		n += getHIDensity(position);
	}
