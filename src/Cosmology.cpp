#include "crpropa/Cosmology.h"
#include "crpropa/Units.h"
#include "crpropa/Common.h"

#include <vector>
#include <cmath>
#include <stdexcept>

namespace crpropa {

/**
 @class Cosmology
 @brief Cosmology calculations
 */
struct Cosmology {
	long double H0; // Hubble parameter at z=0
	long double omegaM; // matter density parameter
	long double omegaL; // vacuum energy parameter

	static const int n;
	static const long double zmin;
	static const long double zmax;

	std::vector<long double> Z;  // redshift
	std::vector<long double> Dc; // comoving distance [m]
	std::vector<long double> Dl; // luminosity distance [m]
	std::vector<long double> Dt; // light travel distance [m]

	void update() {
		long double dH = c_light / H0; // Hubble distance

		std::vector<long double> E(n);
		E[0] = 1;

		// Relation between comoving distance r and redshift z (cf. J.A. Peacock, Cosmological physics, p. 89 eq. 3.76)
		// dr = c / H(z) dz, integration using midpoint rule
		long double dlz = log10(zmax) - log10(zmin);
		for (int i = 1; i < n; i++) {
			Z[i] = zmin * pow(10, i * dlz / (n - 1)); // logarithmic even spacing
			long double dz = (Z[i] - Z[i - 1]); // redshift step
			E[i] = sqrt(omegaL + omegaM * pow_integer<3>(1 + Z[i]));
			Dc[i] = Dc[i - 1] + dH * dz * (1 / E[i] + 1 / E[i - 1]) / 2;
			Dl[i] = (1 + Z[i]) * Dc[i];
			Dt[i] = Dt[i - 1]
					+ dH * dz
							* (1 / ((1 + Z[i]) * E[i])
									+ 1 / ((1 + Z[i - 1]) * E[i - 1])) / 2;
		}
	}

	Cosmology() {
        // Cosmological parameters (K.A. Olive et al. (Particle Data Group), Chin. Phys. C, 38, 090001 (2014))
		H0 = 67.3 * 1000 * meter / second / Mpc; // default values
		omegaM = 0.315;
		omegaL = 1 - omegaM;

		Z.resize(n);
		Dc.resize(n);
		Dl.resize(n);
		Dt.resize(n);

		Z[0] = 0;
		Dc[0] = 0;
		Dl[0] = 0;
		Dt[0] = 0;

		update();
	}

	void setParameters(long double h, long double oM) {
		H0 = h * 1e5 / Mpc;
		omegaM = oM;
		omegaL = 1 - oM;
		update();
	}
};

const int Cosmology::n = 1000;
const long double Cosmology::zmin = 0.0001;
const long double Cosmology::zmax = 100;

static Cosmology cosmology; // instance is created at runtime

void setCosmologyParameters(long double h, long double oM) {
	cosmology.setParameters(h, oM);
}

long double hubbleRate(long double z) {
	return cosmology.H0
			* sqrt(cosmology.omegaL + cosmology.omegaM * pow(1 + z, 3));
}

long double omegaL() {
	return cosmology.omegaL;
}

long double omegaM() {
	return cosmology.omegaM;
}

long double H0() {
	return cosmology.H0;
}

long double comovingDistance2Redshift(long double d) {
	if (d < 0)
		throw std::runtime_error("Cosmology: d < 0");
	if (d > cosmology.Dc.back())
		throw std::runtime_error("Cosmology: d > dmax");
	return interpolate(d, cosmology.Dc, cosmology.Z);
}

long double redshift2ComovingDistance(long double z) {
	if (z < 0)
		throw std::runtime_error("Cosmology: z < 0");
	if (z > cosmology.zmax)
		throw std::runtime_error("Cosmology: z > zmax");
	return interpolate(z, cosmology.Z, cosmology.Dc);
}

long double luminosityDistance2Redshift(long double d) {
	if (d < 0)
		throw std::runtime_error("Cosmology: d < 0");
	if (d > cosmology.Dl.back())
		throw std::runtime_error("Cosmology: d > dmax");
	return interpolate(d, cosmology.Dl, cosmology.Z);
}

long double redshift2LuminosityDistance(long double z) {
	if (z < 0)
		throw std::runtime_error("Cosmology: z < 0");
	if (z > cosmology.zmax)
		throw std::runtime_error("Cosmology: z > zmax");
	return interpolate(z, cosmology.Z, cosmology.Dl);
}

long double lightTravelDistance2Redshift(long double d) {
	if (d < 0)
		throw std::runtime_error("Cosmology: d < 0");
	if (d > cosmology.Dt.back())
		throw std::runtime_error("Cosmology: d > dmax");
	return interpolate(d, cosmology.Dt, cosmology.Z);
}

long double redshift2LightTravelDistance(long double z) {
	if (z < 0)
		throw std::runtime_error("Cosmology: z < 0");
	if (z > cosmology.zmax)
		throw std::runtime_error("Cosmology: z > zmax");
	return interpolate(z, cosmology.Z, cosmology.Dt);
}

long double comoving2LightTravelDistance(long double d) {
	if (d < 0)
		throw std::runtime_error("Cosmology: d < 0");
	if (d > cosmology.Dc.back())
		throw std::runtime_error("Cosmology: d > dmax");
	return interpolate(d, cosmology.Dc, cosmology.Dt);
}

long double lightTravel2ComovingDistance(long double d) {
	if (d < 0)
		throw std::runtime_error("Cosmology: d < 0");
	if (d > cosmology.Dt.back())
		throw std::runtime_error("Cosmology: d > dmax");
	return interpolate(d, cosmology.Dt, cosmology.Dc);
}

} // namespace crpropa
