...
namespace crpropa {
	void PropagationBP::tryStep(const Y &y, Y &out, Y &error, double h,
			ParticleState &particle, double z, double q, double pabs) const {
		out = dY(y.x, y.u, h, z, q, pabs);  // 1 step with h

		Y outHelp = dY(y.x, y.u, h/2, z, q, pabs);  // 2 steps with h/2
		Y outCompare = dY(outHelp.x, outHelp.u, h/2, z, q, pabs);
	...
	PropagationBP::Y PropagationBP::dY(Vector3d pos, Vector3d dir, double step,
			double z, double q, double pabs) const {
		...
		// Boris help vectors using exact momentum magnitude
		Vector3d t = (pabs > 0.) ? B * q * step / (2. * pabs) : Vector3d(0, 0, 0);
	...
	void PropagationBP::process(Candidate *candidate) const {
		...
		double pabs = current.getMomentumExact().getR();
		...
		if (minStep == maxStep){
			yOut = dY(yIn.x, yIn.u, step, z, q, pabs);
		} else {
			step = clip(candidate->getNextStep(), minStep, maxStep);
			newStep = step;
			...
			while (true) {
				tryStep(yIn, yOut, yErr, step, current, z, q, pabs);
