...
EMPairProduction::EMPairProduction(ref_ptr<PhotonField> photonField, bool haveElectrons, long double thinning, long double limit) {
...
void EMPairProduction::setLimit(long double limit) {
	this->limit = limit;
}

void EMPairProduction::setThinning(long double thinning) {
...
class PPSecondariesEnergyDistribution {
	private:
		std::vector<long double> tab_s;
		std::vector< std::vector<long double> > data;
		size_t N;

	public:
		// differential cross section for pair production for x = Epositron/Egamma, compare Lee 96 arXiv:9604098
		long double dSigmadE_PPx(long double x, long double beta) {
			long double A = (x / (1. - x) + (1. - x) / x );
			long double B =  (1. / x + 1. / (1. - x) );
			long double y = (1 - beta * beta);
		...
		PPSecondariesEnergyDistribution() {
			N = 1000;
			size_t Ns = 1000;
			long double s_min = 4 * mec2 * mec2;
			long double s_max = 1e23 * eV * eV;
			long double dls = log(s_max / s_min) / Ns;
			data = std::vector< std::vector<long double> >(Ns, std::vector<long double>(N));
			tab_s = std::vector<long double>(Ns + 1);

			for (size_t i = 0; i < Ns + 1; ++i)
				tab_s[i] = s_min * exp(i*dls); // tabulate s bin borders

			for (size_t i = 0; i < Ns; i++) {
				long double s = s_min * exp(i*dls + 0.5*dls);
				long double beta = sqrt(1 - s_min/s);
				long double x0 = (1 - beta) / 2;
				long double dx = log((1 + beta) / (1 - beta)) / N;

				// cumulative midpoint integration
				std::vector<long double> data_i(1000);
				data_i[0] = dSigmadE_PPx(x0, beta) * expm1(dx);
				for (size_t j = 1; j < N; j++) {
					long double x = x0 * exp(j*dx + 0.5*dx);
					long double binWidth = exp((j+1)*dx)-exp(j*dx);
		...
		long double sample(long double E0, long double s) {
			...
			long double s_min = 4. * mec2 * mec2;
			long double beta = sqrtl(1. - s_min / s);
			long double x0 = (1. - beta) / 2.;
			long double dx = log((1 + beta) / (1 - beta)) / N;
			long double binWidth = x0 * (exp(j*dx) - exp((j-1)*dx));
			if (random.rand() < 0.5)
...
void EMPairProduction::performInteraction(Candidate *candidate) const {
	...
	long double lo = std::max(4 * mec2 * mec2, tabs[j-1]);  // first s-tabulation point below min(s_kin) = (2 me c^2)^2; ensure physical value
	long double hi = tabs[j];
	long double s = lo + random.rand() * (hi - lo);

	// sample electron / positron energy
	static PPSecondariesEnergyDistribution interpolation;
	long double Ee = interpolation.sample(E, s);
	long double Ep = E - Ee;
	long double f = Ep / E;
