...
void TF17Field::set_B1_disk(const long double B1){ B1_disk = B1; }
void TF17Field::set_z1_disk(const long double z1){ z1_disk = z1; }
void TF17Field::set_r1_disk(const long double r1){ r1_disk = r1; }
void TF17Field::set_H_disk(const long double H){ H_disk = H; }
void TF17Field::set_L_disk(const long double L){ L_disk = L; }
void TF17Field::set_a_disk(const long double a){ a_disk = a; }
void TF17Field::set_phi_star_disk(const long double phi){ phi_star_disk = phi; }

void TF17Field::set_B1_halo(const long double B1){ B1_halo = B1; }
void TF17Field::set_z1_halo(const long double z1){ z1_halo = z1; }
void TF17Field::set_L_halo(const long double L){ L_halo = L; }
void TF17Field::set_a_halo(const long double a){ a_halo = a; }
void TF17Field::set_phi_star_halo(const long double phi){ phi_star_halo = phi; }

void TF17Field::set_Hp(const long double H){ H_p = H; }
void TF17Field::set_Lp(const long double L){ L_p = L; }
void TF17Field::set_p0(const long double p0){
    p_0 = p0; 
...
Vector3d TF17Field::getField(const Vector3d& pos) const {
	...
	long double phi = M_PI - pos.getPhi(); // azimuth in our convention
	// long double cosPhi = pos.x / r;
	long double cosPhi = cos(phi);
	// long double sinPhi = pos.y / r;
...
Vector3d TF17Field::getDiskField(const long double& r, const long double& z, const long double& phi, const long double& sinPhi, const long double& cosPhi) const {
	Vector3d b(0.);
    long double B_r = 0;
    long double B_phi = 0;
    long double B_z = 0;
    ...
    if (disk_model == TF17DiskModel::Ad1) { // ==========================================================
        if (r > r1_disk) {
            long double z1_disk_z = (1. + a_disk * r1_disk * r1_disk) / (1. + a_disk * r * r); // z1_disk / z
            // B components in (r, phi, z)
            long double B_r0 = radialFieldScale(B1_disk, phi_star_disk, z1_disk_z*z, phi, r, z);
            ...
            // and thus z = z1
            long double phi1_disk = shiftedWindingFunction(r1_disk, z) + phi_star_disk;
            long double B_amp = B1_disk * exp(-fabs(z) / H_disk);
        ...
        if ( r > epsilon ) {
            long double r1_disk_r = r1_disk / r;
            long double z1_disk_z = 5. / (r1_disk_r*r1_disk_r + 4./sqrt(r1_disk_r)); // z1_disk / z -> remove z dependancy
            long double B_r0 = radialFieldScale(B1_disk, phi_star_disk, z1_disk_z*z, phi, r, z);
            B_r = r1_disk_r * z1_disk_z * B_r0;
            B_z = -0.4 * r1_disk_r / r * z1_disk_z* z1_disk_z * z * (r1_disk_r*r1_disk_r - 1./sqrt(r1_disk_r)) * B_r0;
        } else {
            long double z1_disk_z = 5. * r*r / (r1_disk*r1_disk); // z1_disk / z -> remove z dependancy
            long double B_r0 = radialFieldScale(B1_disk, phi_star_disk, z1_disk_z*z, phi, r, z);
        ...
        long double z_sign = z >= 0 ? 1. : -1.;
        long double z_abs = fabs(z);
        if ( z_abs > epsilon ) {
            long double z1_disk_z = z1_disk / z_abs;
            long double r1_disk_r = 1.5 / (sqrt(z1_disk_z) + 0.5/z1_disk_z); // r1_disk / r
            long double F_r = r1_disk_r*r  <= L_disk ? 1. : exp(1. - r1_disk_r*r/L_disk);
        // simplication of the equation in the cosinus
            long double B_z0 = z_sign * B1_disk * F_r * cos(phi - shiftedWindingFunction(r, z) - phi_star_disk);
            B_r = -0.5/1.5 * r1_disk_r * r1_disk_r * r1_disk_r * r / z_abs * (sqrt(z1_disk_z) - 1/z1_disk_z) * B_z0;
            B_z = z_sign * r1_disk_r * r1_disk_r * B_z0;
        } else {
            long double z_z1_disk = z_abs / z1_disk;
            long double r1_disk_r = 1.5 * sqrt(z_abs / z1_disk); // r1_disk / r
            long double F_r = r1_disk_r*r  <= L_disk ? 1. : exp(1. - r1_disk_r*r/L_disk);
            long double B_z0 = z_sign * B1_disk * F_r * cos(phi - shiftedWindingFunction(r, z) - phi_star_disk);
...
Vector3d TF17Field::getHaloField(const long double& r, const long double& z, const long double& phi, const long double& sinPhi, const long double& cosPhi) const {
    int m;

	Vector3d b(0.);
	long double r1_halo_r =  (1. + a_halo * z1_halo * z1_halo) / (1. + a_halo * z * z);
	// B components in (r, phi, z)
    long double B_z0;
    ...
    if (halo_model == TF17HaloModel::C0) { // m = 0
        ...
        // simplication of the equation in the cosinus
        long double phi_prime = phi - shiftedWindingFunction(r, z) - phi_star_halo;
	...
	long double B_r = 2 * a_halo * r1_halo_r * r1_halo_r * r * z / (1. + a_halo * z * z) * B_z0;
	long double B_z = r1_halo_r * r1_halo_r * B_z0;
	long double B_phi = azimuthalFieldComponent(r, z, B_r, B_z);
...
long double TF17Field::azimuthalFieldComponent(const long double& r, const long double& z, const long double& B_r, const long double& B_z) const {
	long double r_ = r / L_p;
    long double rscale = r > epsilon ? r_ * exp(-r_) / (1 - exp(-r_)) : 1 - r_/2. - r_*r_/12. ;
	long double B_phi = cot_p0 / zscale(z) * rscale * B_r;
    B_phi = B_phi - 2 * z * r / (H_p * H_p) / zscale(z) * shiftedWindingFunction(r, z) * B_z;
	return B_phi;
}

long double TF17Field::radialFieldScale(const long double& B1, const long double& phi_star, const long double& z1, const long double& phi, const long double& r, const long double& z) const {
    // simplication of the equation in the cosinus
    long double phi_prime = phi - shiftedWindingFunction(r, z) - phi_star;
...
long double TF17Field::shiftedWindingFunction(const long double& r, const long double& z) const {
    return cot_p0 * log(1 - exp(-r / L_p) + epsilon) / zscale(z);
}

long double TF17Field::zscale(const long double& z) const {
