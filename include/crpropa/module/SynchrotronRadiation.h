...
private:
	...
	long double Brms; ///< Brms value in case no MagneticField is specified
	long double limit; ///< fraction of energy loss length to limit the next step
	long double thinning; ///< thinning parameter for weighted-sampling (maximum 1, minimum 0)
	...
	long double secondaryThreshold; ///< threshold energy for secondary photons
	std::vector<long double> tabx; ///< tabulated fraction E_photon/E_critical from 10^-6 to 10^2 in 801 log-spaced steps
	std::vector<long double> tabCDF; ///< tabulated CDF of synchrotron spectrum
...
public:
	 ...
	 */
	SynchrotronRadiation(ref_ptr<MagneticField> field, bool havePhotons = false, long double thinning = 0, int nSamples = 0, long double limit = 0.1);
	/** Constructor
	 ...
	 @param limit			step size limit as fraction of mean free path
	 */
	SynchrotronRadiation(long double Brms = 0, bool havePhotons = false, long double thinning = 0, int nSamples = 0, long double limit = 0.1);
	 ...
	 */
	void setSecondaryThreshold(long double threshold);
	void setInteractionTag(std::string tag);
	ref_ptr<MagneticField> getField();

	long double getBrms();
	bool getHavePhotons();
	long double getThinning();
	long double getLimit();
	int getMaximumSamples();
	long double getSecondaryThreshold() const;
