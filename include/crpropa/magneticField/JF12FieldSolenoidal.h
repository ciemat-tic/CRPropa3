...
private:
	long double zS; // height parameter of the modified X-field, field lines are parabolic for fabs(z) < zS
	long double phi0Arms[11]; // azimuth angles in [-pi,pi] at which the dividing 8 spirals of the disk field intersect the (r1 = 5kpc)-ring at indices 1 to 8, remaining angles periodically filled.

	long double phi0; // lower bound for azimuth phi integration to restore solenoidality of spiral field transition, arbitrary parameter
	...
	long double phiCoeff[10]; // only 10 since the array holds the results for the integral from phi0Arms[0] to phi0Arms[1]; phi0Arms[0] to phi0Arms[2] etc.
	long double corr; // correction term for enforcing H(phi0) = 0 afterwards which sets the lower boundary of the integration to phi0
...
*/
	JF12FieldSolenoidal(long double delta = 3 * kpc, long double zs = 0.5 * kpc);
...
/** @brief Adjust the transition width of the disk field
	...
	@return Void
*/
	void setDiskTransitionWidth(long double delta);
	long double getDiskTransitionWidth() const;
...
/** @brief Adjust the scale heigth of the X field
	...
	@return Void
*/
	void setXScaleHeight(long double zs);
	long double getXScaleHeight() const;

	Vector3d getXField(const long double& r, const long double& z, const long double& sinPhi, const long double& cosPhi) const; // override old X and spiral field
...
*/
	long double getDiskTransitionPolynomial(const long double& r) const;
...
*/
	long double getDiskTransitionPolynomialDerivative(const long double& r) const;
...
/** @brief Evaluate an angular azimuth integral over the piecewise constant spiral field strengths at r_1 = 5 kpc. This integral is needed to restore the solenoidality of the spiral field in the transition regions.
	@param r 	Distance of the current position to the z axis in the usual galactocentric cylindrical coordinates. Should be non-negative.
	@param phi	Azimuth angle of the current position in galactocentric cylindrical coordinates. Can be any long double.
...
*/
	long double getHPhiIntegral(const long double& r, const long double& phi) const;
...
*/
	long double getSpiralFieldStrengthConstant(const long double& r, const long double& phi) const;
};
/** @} */
